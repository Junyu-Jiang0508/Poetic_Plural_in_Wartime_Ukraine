...
2. ``fig_narr_2_author_trajectories.{pdf,png}`` — each roster author drawn
   ...
   labels for the caterpillar top-5 / bottom-5 poets (fig.\\,8 extremes).
...
from utils.author_meta import load_author_meta
...
DEFAULT_POEM_CELL = ROOT / "data" / "pronoun_cell_counts.csv"
...
# fig2 highlighted set (fallback when posterior summaries are unavailable).
...
def _read_inputs(poem_path: Path, roster_path: Path, authors_path: Path):
    ...
    if "2pl_vy_true_plural" not in df.columns and "2pl" in df.columns:
        df["2pl_vy_true_plural"] = df["2pl"]
    if "author_id" in df.columns and "author" not in df.columns:
    ...
    if "author_id" in roster.columns and "author" not in roster.columns:
        roster["author"] = roster["author_id"]
    authors = pd.read_csv(authors_path) if authors_path.is_file() else pd.DataFrame()
    if not authors.empty and "author_id" in authors.columns and "author" not in authors.columns:
        authors["author"] = authors["author_id"]
    cov = load_author_meta()
    if "author_id" in cov.columns and "author" not in cov.columns:
        cov = cov.rename(columns={"author_id": "author"})
...
# fig2 only: replace caterpillar bottom-5 fallers whose trajectory arrow is too
# short to read on the (1sg, 1pl) plane.
FIG2_FALLER_SWAPS: dict[str, str] = {
    "Yury Solomko": "Olena Boryshpolets",
}


def _caterpillar_extreme_authors(
    ...
    """Return the bottom-``bottom_n`` and top-``top_n`` authors from the 1pl caterpillar."""
    d = authors[(authors["language_stratum"] == stratum) & (authors["cell"] == cell)].copy()
    if d.empty:
        return []
    d = d.sort_values("author_total_period_shift_mean_log_mu", ascending=True)
    return d.head(bottom_n)["author"].tolist() + d.tail(top_n)["author"].tolist()


def _fig2_highlight_authors(authors: pd.DataFrame) -> list[str]:
    """Caterpillar top/bottom-5, with fig2-specific faller swaps for readability."""
    highlight = _caterpillar_extreme_authors(authors)
    fallers = highlight[:5]
    risers = highlight[5:]
    for drop, add in FIG2_FALLER_SWAPS.items():
        if drop in fallers and add not in fallers and add not in risers:
            fallers = [add if author == drop else author for author in fallers]
    return fallers + risers


def _fig2_highlight_label(author: str, cov_by_author: pd.DataFrame) -> str:
    """Single-line label for a highlighted author (name or anonymized code)."""
    row = cov_by_author.loc[author]
    label = _disp(author)
    if is_at_frontline(
...
def _fig2_side_rail_labels(
    ax,
    hi_rows: pd.DataFrame,
    highlight_list: list[str],
    cov_by_author: pd.DataFrame,
) -> None:
    """Place caterpillar-extreme labels on left (fallers) / right (risers) side rails."""
    def _collect(authors: list[str]) -> list[tuple[str, float, float]]:
        items: list[tuple[str, float, float]] = []
        for author in authors:
            sub = hi_rows[hi_rows["author"] == author]
            if sub.empty:
                continue
            r = sub.iloc[0]
            items.append((
                author,
                float(r[f"share_1sg_{POST_PERIOD}"]),
                float(r[f"share_1pl_{POST_PERIOD}"]),
            ))
        items.sort(key=lambda t: t[2])
        return items

    rails = (
        ("left", highlight_list[:5], _collect(highlight_list[:5])),
        ("right", highlight_list[5:], _collect(highlight_list[5:])),
    )
    for side, _, items in rails:
        if not items:
            continue
        n = len(items)
        y_fracs = np.linspace(0.11, 0.89, n)
        x_text = -0.055 if side == "left" else 1.055
        ha = "right" if side == "left" else "left"
        for (author, x1, y1), y_frac in zip(items, y_fracs):
            row = cov_by_author.loc[author]
            c = location_color(
                row.get("mobilized"), row.get("in_ukraine_wartime"),
            ...
            ax.annotate(
                _fig2_highlight_label(author, cov_by_author),
                xy=(x1, y1),
                xycoords="data",
                ...
                ha=ha,
                va="center",
                bbox=dict(boxstyle="round,pad=0.20", fc="#FFFCF5",
...
def fig2_author_trajectories(
    ...
    fig.subplots_adjust(left=0.16, right=0.84)
    ax.set_facecolor("white")
    ...
    if authors is not None and not authors.empty:
        highlight_list = _fig2_highlight_authors(authors)
    else:
        highlight_list = HIGHLIGHT_AUTHORS
    annotate_set = set(highlight_list)
    ...
    _cov_roster = load_author_meta()
    if "author_id" in _cov_roster.columns:
        _cov_roster = _cov_roster.rename(columns={"author_id": "author"})
    ...
    _fig2_side_rail_labels(ax, hi_rows, highlight_list, cov_by_author)
    ...
    log.info(
        "Wrote fig_narr_2_author_trajectories (n=%d authors, %d highlighted)",
        len(wide), len(highlight_list),
    )
...
def fig8_caterpillar_1pl_focused(authors: pd.DataFrame, out_dir: Path,
    ...
    cov_path = ROOT / "data" / "author_meta.csv"
    _sit = pd.read_csv(cov_path, low_memory=False)
    if "author_id" in _sit.columns:
        _sit = _sit.rename(columns={"author_id": "author"})
    _sit["in_ukraine_wartime"] = _sit["in_ukraine_2022"].map(lambda x: "yes" if int(x) == 1 else "no")
    _sit["mobilized"] = "no"
    _sit = _sit.set_index("author")
...
def main() -> None:
    ...
    if args.anonymize:
        ...
        roster_csv = ROOT / "data" / "author_meta.csv"
        ...
        fig2_author_trajectories(df, roster, cov, out_dir, authors=authors)
