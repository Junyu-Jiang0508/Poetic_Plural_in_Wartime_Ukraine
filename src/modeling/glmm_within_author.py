...
def fit_hierarchical_covariate_adjusted_per_cell(
    ...
    *,
    ...
    covariates = load_author_meta()
    if covariates.empty:
        warnings.warn(
            "Author covariates CSV is empty; covariate-adjusted Q2 cannot run. "
            "Populate data/author_meta.csv (see docs/PREREGISTRATION.md §10)."
        )
        return pd.DataFrame()
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "data/author_meta.csv populated for the authors in scope."
        ),
    )
    parser.add_argument(
        ...
        help=(
            ...
            "as safe in utils.author_meta.SAFE_FOR_PERIOD_SLOPE_PREDICTORS."
        ),
    )
    ...
    if poem_table_path.is_file() and is_shipped_poem_table(poem_table_path):
        poem_cell = load_shipped_poem_table(poem_table_path)
        roster_authors = load_roster_authors(args.roster.resolve() if args.roster else None)
        if roster_authors is not None:
            poem_cell = poem_cell[poem_cell["author"].astype(str).isin(roster_authors)].copy()
    else:
    ...
    with (out_dir / "README.md").open("w", encoding="utf-8") as f:
        ...
        if args.include_author_covariates:
            f.write(
                ...
                "invariant author covariates loaded from `data/author_meta.csv`.\n"
